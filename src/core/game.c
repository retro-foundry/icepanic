...
enum {
    ...
    BLOCK_SPEED_FP = 2 * FP_ONE,
#else
    BLOCK_SPEED_FP = (9 * FP_ONE) / 4,
#endif
#if defined(ICEPANIC_AMIGA_OCS_ARCADE_CORE)
    PLAYER_DIR_BUFFER_TICKS = 6,
#else
...
static void update_timer_danger_state(GameState *gs) {
    ...
    const int pulse_cooldown_floor = -clampi(pulse_interval_ticks - pulse_ticks, 1, GAME_FIXED_TICK_HZ);
    ...
    if (gs->round_time_ticks <= danger_threshold_ticks) {
        ...
        } else if (gs->timer_danger_pulse_ticks <= pulse_cooldown_floor) {
            gs->timer_danger_pulse_ticks = pulse_ticks;
            emit_event(gs, GAME_EVENT_TIMER_DANGER_PULSE);
        } else {
...
static int respawn_enemy_pixel_tile(GamePixelFp pixel_fp, int max_tile) {
    return clampi((int)((pixel_fp + (GamePixelFp)(TILE_FP / 2)) / (GamePixelFp)TILE_FP), 0, max_tile);
}
...
static int respawn_min_enemy_distance(const GameState *gs, int x, int y) {
    ...
    for (int i = 0; enemy_mask != 0u && i < gs->enemy_count; ++i, enemy_mask >>= 1) {
        ...
        dist = abs_int_fast(enemy->tile_x - x) + abs_int_fast(enemy->tile_y - y);
        pixel_dist =
            abs_int_fast(respawn_enemy_pixel_tile(enemy->pixel_fp_x, GAME_GRID_WIDTH - 1) - x) +
            abs_int_fast(respawn_enemy_pixel_tile(enemy->pixel_fp_y, GAME_GRID_HEIGHT - 1) - y);
        if (pixel_dist < dist) {
            dist = pixel_dist;
        }
        if (dist < min_dist) {
...
static int respawn_tile_score(const GameState *gs, int x, int y, int sx, int sy) {
    ...
    const int spawn_dist = abs_int_fast(x - sx) + abs_int_fast(y - sy);
    if (enemy_dist < 0) {
        return -30000;
    }
    if (enemy_dist > 16) {
    ...
    return enemy_dist * 180 + exits * 24 + spawn_dist;
}

static void find_safe_respawn_tile(const GameState *gs, int *out_x, int *out_y) {
    enum {
        RESPAWN_IDEAL_ENEMY_DISTANCE = 6,
        RESPAWN_HARD_ENEMY_DISTANCE = 3
    };
...
#if defined(ICEPANIC_AMIGA_OCS_ARCADE_CORE)
static int ocs_enemy_speed_fp(int speed_fp) {
    return clampi(speed_fp, FP_ONE, 2 * FP_ONE);
}
#endif
...
void game_start_round(GameState *gs, int round_index) {
    ...
    for (int i = 0; i < gs->enemy_count; ++i) {
        ...
        if (i >= ghost_start) {
            ...
            enemy->speed_fp = clampi(
                gs->round_config.enemy_speed_fp + map_pressure_speed_bonus_fp + (FP_ONE / 8),
                FP_ONE,
                2 * FP_ONE);
...
#if defined(ICEPANIC_AMIGA_OCS_ARCADE_CORE)
        enemy->speed_fp = ocs_enemy_speed_fp(enemy->speed_fp);
#endif

...
ICE_INLINE void wait_blitter(void) {
    ...
    while ((CREGS->dmaconr & DMAF_BLTDONE) != 0) {
        ...
        if ((g_probe_blitter_poll_phase & 63u) == 0u &&
...
ICE_INLINE int clamp_int_probe(int value, int min_value, int max_value) {
    if (value < min_value) {
        return min_value;
    }
    if (value > max_value) {
...
ICE_INLINE UWORD read_raster_line_probe(void) {
    UWORD vpos_hi;
    UWORD vhpos_a;
    UWORD vhpos_b;
    do {
...
static UBYTE *probe_hw_title_sparkle_slot(UWORD *cop, WORD slot) {
    return probe_hw_enemy_slot(cop, slot);
}
...
static void build_probe_title_hw_sparkle_streams(UWORD *cop, const UBYTE *tiles) {
    WORD p;
    if (!cop || !tiles) {
        return;
    }
    for (p = 0; p < PROBE_HW_SPARKLE_COUNT; ++p) {
        build_probe_sparkle_hw_sprite((UWORD *)probe_hw_title_sparkle_slot(cop, p), tiles);
    }
}
...
static void patch_probe_hw_sprite_ptrs(WORD cop_index, UBYTE *sprite_bank, UWORD *cop, WORD mode) {
    ...
    for (p = 0; p < PROBE_HW_SPRITE_COUNT; ++p) {
        ...
        ULONG addr;
        if (mode == PROBE_HW_PTR_MODE_TITLE &&
            cop &&
            p >= PROBE_HW_SPARKLE_FIRST_CHANNEL) {
            ptr = probe_hw_title_sparkle_slot(cop, (WORD)(p - PROBE_HW_SPARKLE_FIRST_CHANNEL));
        }
        addr = (ULONG)ptr;
...
static void update_probe_title_hw_sparkles(UWORD *cop, ULONG tick, BOOL active) {
    ...
    for (count = 0; count < PROBE_HW_SPARKLE_COUNT; ++count) {
        UWORD *sprite = (UWORD *)probe_hw_title_sparkle_slot(cop, count);
        sprite[0] = 0x0000;
        sprite[1] = 0x0000;
    }
    if (!active) {
...
static void patch_probe_copper_reflection(WORD cop_index, ULONG tick) {
    ...
    *g_probe_copper_reflection_first[cop_index] = (UWORD)(scroll | (scroll << 4));
    ...
    for (line = 1; line < PROBE_PAL_REFLECTION_LINES; ++line) {
        scroll = wave[(WORD)((phase + (ULONG)(line / 3)) & 63UL)];
...
static void draw_probe_enter_hi_score_slot_delta(UBYTE *screen,
                                                 const ProbeHighScoreEntryState *entry,
                                                 WORD slot_index) {
    const WORD panel_x0 = 54;
    const WORD panel_x1 = 265;
    const WORD slot_w = 28;
    ...
    char letter[2];
    WORD glyph_w;
    if (!screen || !entry || !entry->active || slot_index < 0 || slot_index >= 3) {
        return;
    }
    slot_x = (WORD)(slots_x0 + slot_index * (slot_w + slot_spacing));
    selected = (slot_index == entry->cursor) ? TRUE : FALSE;
    draw_rect_probe(screen,
                    (WORD)(slot_x - 5),
                    (WORD)(slots_y - 8),
                    (WORD)(slot_x + slot_w + 4),
                    (WORD)(slots_y + slot_h + 8),
                    1);
    letter[0] = entry->initials[slot_index];
    letter[1] = '\0';
    glyph_w = probe_text_width_scaled(letter, 3);
    draw_probe_card_frame(screen, slot_x, slots_y, slot_w, slot_h, selected ? 29 : 17, selected);
    draw_probe_text_scaled_outlined(screen,
...
static void draw_probe_enter_hi_score_delta(UBYTE *screen,
                                            const ProbeHighScoreEntryState *entry,
                                            WORD previous_key) {
    WORD previous_cursor;
    if (!screen || !entry || !entry->active) {
        return;
    }
    previous_cursor = (WORD)((UWORD)previous_key % 3u);
    if (previous_cursor >= 0 && previous_cursor < 3) {
        draw_probe_enter_hi_score_slot_delta(screen, entry, previous_cursor);
    }
    if (entry->cursor != previous_cursor) {
        draw_probe_enter_hi_score_slot_delta(screen, entry, entry->cursor);
    }
}
...
static BOOL draw_probe_ui_delta(UBYTE *screen, const GameState *game, WORD previous_selection) {
    if (g_probe_high_score_entry.active) {
        draw_probe_enter_hi_score_delta(screen, &g_probe_high_score_entry, previous_selection);
        return TRUE;
    }
...
static BOOL probe_bob_overlaps_blitted_bobs(const ProbeDynamicBob *current,
                                            uint32_t blitted_lo,
                                            UBYTE blitted_hi,
                                            const ProbeDynamicBob *bob) {
    uint32_t scan_lo;
    UBYTE scan_hi;
    WORD slot;
    if (!current || !bob || !bob->valid) {
        return FALSE;
    }
    scan_lo = blitted_lo;
    for (slot = 0; scan_lo != 0u && slot < 32; ++slot, scan_lo >>= 1) {
        if ((scan_lo & 1u) != 0u) {
            WORD delta = (WORD)(current[slot].x - bob->x);
            if (delta < 0) delta = (WORD)-delta;
            if (delta < GAME_TILE_SIZE) {
                delta = (WORD)(current[slot].y - bob->y);
                if (delta < 0) delta = (WORD)-delta;
                if (delta < GAME_TILE_SIZE) {
                    return TRUE;
                }
...
static void probe_draw_dynamic_bobs_ordered_dirty(UBYTE *screen,
                                                  const UBYTE *chip_bobs,
                                                  const UBYTE *chip_masks,
                                                  const ProbeDynamicBob *current,
                                                  uint32_t current_lo,
                                                  ...
                                                  uint32_t changed_lo,
                                                  UBYTE changed_hi,
                                                  const uint32_t *static_dirty_rows) {
    uint32_t blitted_lo;
    uint32_t scan_lo;
    UBYTE blitted_hi;
    UBYTE scan_hi;
    ...
    for (slot = 0; scan_lo != 0u && slot < 32; ++slot, scan_lo >>= 1) {
        if ((scan_lo & 1u) != 0u &&
            (((changed_lo & ((uint32_t)1u << (uint32_t)slot)) != 0u) ||
             probe_bob_overlaps_dirty_tiles(static_dirty_rows, &current[slot]) ||
             probe_bob_overlaps_blitted_bobs(current, blitted_lo, blitted_hi, &current[slot]))) {
            blit_shifted_bob_twopass_probe_async(screen, chip_bobs, chip_masks,
                                                 current[slot].sprite, current[slot].x, current[slot].y);
            probe_dynamic_slot_set(slot, &blitted_lo, &blitted_hi);
        }
    }
    scan_hi = current_hi;
    for (slot = 32; scan_hi != 0u && slot < PROBE_DYNAMIC_CACHE_MAX; ++slot, scan_hi >>= 1) {
        if ((scan_hi & 1u) != 0u &&
            (((changed_hi & (UBYTE)(1u << (UBYTE)(slot - 32))) != 0u) ||
             probe_bob_overlaps_dirty_tiles(static_dirty_rows, &current[slot]) ||
             probe_bob_overlaps_blitted_bobs(current, blitted_lo, blitted_hi, &current[slot]))) {
            blit_shifted_bob_twopass_probe_async(screen, chip_bobs, chip_masks,
                                                 current[slot].sprite, current[slot].x, current[slot].y);
...
static void draw_probe_gameplay_dirty(ProbeRenderCache *cache,
    ...
    for (slot = 0; scan_lo != 0u && slot < 32; ++slot, scan_lo >>= 1, prev_scan >>= 1, current_scan >>= 1) {
        ...
        if (!prev_valid || !current_valid ||
            ...
            if (current_valid) {
                ...
                probe_dynamic_slot_set(slot, &changed_lo, &changed_hi);
            }
        }
    }
    }
    ...
    if (draw_any) {
        if (hidden_sensitive) {
            ...
            probe_draw_dynamic_bobs_ordered_dirty(screen,
                                                 chip_bobs,
                                                 chip_masks,
                                                 current,
                                                 current_lo,
...
int main(void) {
    ...
    build_probe_title_hw_sparkle_streams(cop[0], chip_tiles);
    build_probe_title_hw_sparkle_streams(cop[1], chip_tiles);
    update_probe_title_hw_sparkles(cop[0], tick, probe_ui_uses_title(ui_mode));
    update_probe_title_hw_sparkles(cop[1], tick, probe_ui_uses_title(ui_mode));
    ...
    CREGS->dmacon = DMAF_SETCLR | DMAF_MASTER | DMAF_COPPER | DMAF_RASTER | DMAF_BLITTER | DMAF_BLITHOG;
    WaitTOF();
    ...
    while (1) {
        ...
        probe_poll_input_latch(&input_latch, &last_dirs, &last_fire);
        {
            ...
            if (title_sprites_active) {
                if (sprite_ptr_mode[draw_index] != PROBE_HW_PTR_MODE_TITLE) {
                    build_probe_title_hw_sparkle_streams(cop[draw_index], chip_tiles);
                }
                update_probe_title_hw_sparkles(cop[draw_index], tick, TRUE);
                ptr_mode = PROBE_HW_PTR_MODE_TITLE;
            } else if (sprite_ptr_mode[draw_index] != PROBE_HW_PTR_MODE_NULL) {
                update_probe_title_hw_sparkles(cop[draw_index], tick, FALSE);
            }
            ...
            if (title_sprites_active && !sprite_dma_enabled) {
                CREGS->dmacon = DMAF_SETCLR | DMAF_SPRITE;
                sprite_dma_enabled = TRUE;
            } else if (!title_sprites_active && sprite_dma_enabled) {
                CREGS->dmacon = DMAF_SPRITE;
                sprite_dma_enabled = FALSE;
            }
        }

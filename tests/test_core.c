...
static bool test_respawn_prefers_far_tile_when_spawn_is_enemy_crowded(void) {
    const char *name = "respawn_prefers_far_tile_when_spawn_is_enemy_crowded";
    GameState gs;
    InputState none = {0};
    ...
    gs.player_spawn_y = 2;
    gs.enemy_count = 4;
    gs.alive_enemy_count = 4;
    gs.alive_enemy_mask = 0x0Fu;
    for (int i = 0; i < gs.enemy_count; ++i) {
        gs.enemies[i].alive = true;
        gs.enemies[i].state = ENEMY_ROAMING;
        gs.enemies[i].type = ENEMY_TYPE_CHASER;
        gs.enemies[i].speed_fp = FP_ONE_TEST;
    }
    gs.enemies[0].tile_x = 4;
    gs.enemies[0].tile_y = 2;
    gs.enemies[1].tile_x = 2;
    gs.enemies[1].tile_y = 5;
    gs.enemies[2].tile_x = 5;
    gs.enemies[2].tile_y = 5;
    gs.enemies[3].tile_x = 7;
    gs.enemies[3].tile_y = 3;
    for (int i = 0; i < gs.enemy_count; ++i) {
        gs.enemies[i].pixel_fp_x = tile_to_fp_test(gs.enemies[i].tile_x);
        gs.enemies[i].pixel_fp_y = tile_to_fp_test(gs.enemies[i].tile_y);
    }
    gs.enemies[3].pixel_fp_x += (GAME_TILE_SIZE * FP_ONE_TEST) / 2;
    ...
    REQUIRE(name, gs.phase == GAME_PHASE_ROUND_INTRO, "respawn should return to round intro");
    REQUIRE(name, gs.player.alive, "player did not respawn alive");
    REQUIRE(name, gs.player.tile_x != 2 || gs.player.tile_y != 2, "player stayed at the crowded original spawn");
    for (int i = 0; i < gs.enemy_count; ++i) {
        const int dist = abs(gs.enemies[i].tile_x - gs.player.tile_x) + abs(gs.enemies[i].tile_y - gs.player.tile_y);
        REQUIRE(name, dist >= 6, "player did not respawn well away from enemies");
    }
    return true;
}
...
int main(void) {
    ...
    failed += test_respawn_prefers_far_tile_when_spawn_is_enemy_crowded() ? 0 : 1;

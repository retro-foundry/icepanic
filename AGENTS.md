...
- The OCS beta may enable `DMAF_BLITHOG` during its custom display because gameplay is BOB-heavy and the CPU mostly waits on blits. This is allowed only alongside dirty rendering and proper blitter waits; it is not permission to redraw unnecessary actors or full screens.
...
- Gameplay enemies stay on the blitter BOB path in the OCS beta. Do not reintroduce gameplay enemy hardware sprites unless the user explicitly requests that experiment and the memory/timing budget is checked again.
- The unused tail of each already-reserved copperlist may hold title sparkle streams. This is deliberate: it keeps the proven graphics arena size and allocation order unchanged while still keeping sprite DMA data below `$00080000`. Do not grow or move the compact sprite bank for this path unless the memory map is explicitly rebudgeted.
...
- OCS gameplay rendering is dirty-only. Keep the CPU-only per-buffer render cache: mark old/new dynamic BOB tile coverage dirty only when the cached BOB changed, redraw only dirty static cells, and blit only current dynamic BOBs needed for changed actors, restored cells, and draw-order preservation. Gameplay enemies stay in the BOB path. HUD values must be field-delta patched, not full-strip redrawn. Do not restore/redraw every actor every frame and do not clear/redraw the full playfield while gameplay is active. Hardware title-sparkle sprite streams stay title-only, but the copper palette sparkles and reflection wave are global copper effects and should keep animating from the existing copperlist slots.
...
- The old OCS probe is now the primary Amiga beta line and replaces the old ECS release path. Build it with `make -f Makefile.amiga`; the output is `build/amiga/icepanic-v0.9.1-beta.3-amiga-ocs-pal.adf`.
